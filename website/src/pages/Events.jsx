...
export default function Events() {
  ...
  // Fetch specific event if ID is in the URL
  useEffect(() => {
    if (id) {
  ...
  // Categories MUST match the lowercase values in your Sanity screenshot
  const categories = [
    { label: 'All', value: 'all' },
  ...
  // VIEW 1: Detailed Event Page (Triggered by /event/:id)
  if (id && singleEvent) {
    return (
  ...
  // VIEW 2: General Events List (Triggered by /events)
  return (
    <div className="pt-32 p-8 max-w-7xl mx-auto text-white min-h-screen">
      <h1 className="text-6xl font-black uppercase italic mb-4 tracking-tighter text-center md:text-left">SOFEA Events</h1>
      
      {/* Category Filter Bar */}
      <div className="flex gap-3 mb-12 overflow-x-auto pb-4 scrollbar-hide justify-start md:justify-start">
        {categories.map(cat => (
          <button 
            key={cat.value} 
