...
const NavOverlay = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <div 
      className={`fixed inset-0 z-[10000] bg-[#0a0a0a] transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] flex items-center px-10 md:px-32 ${
        isOpen 
                ...
                <div key={link.name} className="h-auto overflow-visible relative">
                  <Link 
                    ...
                    className={`
                      ...
                      text-3xl md:text-5xl font-black uppercase tracking-widest
                      pointer-events-auto
                      ${isOpen ? `menu-typewriter delay-${index + 1}` : 'opacity-0'}
                      ${isActive ? "text-[#bc9c22]" : "text-white"} 
                    `}
                  >
                    {link.name}
                  </Link>
